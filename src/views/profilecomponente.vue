<template>
  <section class="wrapper">
    ...
    <div class="grid kpis">
      ...
      <KpiCard icon="💾" :value="`${stats.usedStorage}GB`" label="Armazenamento" />
    </div>
    ...
    <div class="grid two-cols">
      ...
      <Card title="Dados Pessoais">
        <div class="form-grid">
          <Field label="Nome Completo"><input v-model="profile.name" type="text" /></Field>
          <Field label="E-mail"><input v-model="profile.email" type="email" /></Field>
          <Field label="Telefone"><input v-model="profile.phone" type="tel" /></Field>
          <Field label="Empresa"><input v-model="profile.company" type="text" /></Field>
          <Field label="Bio" class="full">
            <textarea v-model="profile.bio" rows="3" placeholder="Conte-nos um pouco sobre você..."></textarea>
          </Field>
        </div>
      </Card>
      ...
      <Card title="Plano Atual">
        ...
        <div class="meter">
          ...
          <div class="hint">{{ stats.usedStorage }}GB / {{ plan.storageLimit }}GB</div>
        </div>
        <div class="meter">
        ...
        <button class="btn primary w100" @click="onManage">Gerenciar Plano</button>
      </Card>
    </div>
...
const profile = reactive({
  ...
  bio: '',
  memberSince: '2024',
  avatarUrl: 'https://images.unsplash.com/photo-1502685104226-ee32379fefbe?q=80&w=200&h=200&fit=crop&auto=format',
})

const stats = reactive({ files: 1247, categories: 18, prompts: 342, usedStorage: 45.2 })
const plan = reactive({ name: 'Plano Pro', price: '29,90', storageLimit: 100, promptsLimit: 1000 })
const prefs = reactive({ language: 'Português (Brasil)', timezone: 'GMT-3 (São Paulo)', darkTheme: false })
...
// Computed
const storagePct = computed(() => Math.min(100, Math.round((stats.usedStorage / plan.storageLimit) * 100)))
const promptsPct = computed(() => Math.min(100, Math.round((stats.prompts / plan.promptsLimit) * 100)))
