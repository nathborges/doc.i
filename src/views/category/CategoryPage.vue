<template>
  <UiParentCard :title="`${categoryName}`" :subtitle="categoryDescription" class="w-100 h-100">
    ...
    <template v-slot:action>
      <v-chip size="small" color="primary" variant="tonal">{{ documents.length }} arquivos</v-chip>
    </template>
    ...
    <div v-else-if="documents.length > 0" class="d-flex flex-column position-relative">
      <LoadingOverlay
        :isVisible="categoriesStore.deletingDocument"
        message="Deletando documento..."
      />
      <v-data-table
        :headers="headers"
        :items="documents"
        ...
        hover
        density="comfortable"
        :sort-by="[{ key: 'fileName', order: 'asc' }]"
      >
...
<script setup lang="ts">
  ...
  const tableTitle = computed(() => (isSearchMode.value ? 'Resultados da busca:' : 'Arquivos:'));
