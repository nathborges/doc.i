  ...
  <div class="search">
    <div class="search-container">
      <div class="input-wrapper">
        ...
        />
        <span class="search-icon material-icons" @click="handleSearch"
          >search</span
        >
      </div>
      ...
      <transition name="slide-fade">
        <div v-if="false" class="search-results" :class="{ error: hasError }">
          <span class="close-icon material-icons" @click="closeResults"
            >close</span
          >
      ...
      <div class="search-filters">
        <div
          v-for="(filter, index) in searchFilter"
          :key="index"
          class="search-filter-tag"
        >
          <span class="filter-text">{{ filter }}</span>
          <span
            class="material-icons filter-remove"
...
<script setup>
  ...
  const fullResponse = ref(
    'Encontrei alguns documentos que podem ser relevantes para sua pesquisa. Aqui estão os resultados mais relevantes.'
  )
    ...
    SearchService.ask(query.value)
      .then(data => {
        ...
        if (data.files && data.files.length > removeCount.value) {
          data.files.splice(
            data.files.length - removeCount.value,
            removeCount.value
          )
        }
