<template>
  ...
  <div class="sidebar" :class="{ 'mobile-open': isOpen }">
    ...
    <div class="menu">
      <div v-for="item in menuItems" :key="item.id" class="menu-item title" :class="{ active: isActiveRoute(item.route) }"
        :data-mobile-only="item.mobileOnly" @click="handleMenuClick(item.id)">
      ...
      <div v-if="hasCategories" class="categories-section">
        ...
        <div class="categories-list">
          <div v-for="category in categories" class="category-item" :class="{
            active:
              route.name === 'category' &&
              route.params.name ===
...
@media (min-width: 769px) {
  .menu-item[data-mobile-only='true'] {
    display: none;
  }
}
