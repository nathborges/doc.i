<template>
  <ModalWrapper v-if="isOpen" :key="isOpen" width="520px" title="Upload de Arquivos" @close="closeModal">
...
const saveFailedUploadToStorage = (file, categoryId) => {
...
const uploadFiles = async () => {
  ...
  for (const file of files.value) {
    try {
      ...
      window.showToast(`Enviado: ${file.name}`, 'success')
    } catch (error) {
      saveFailedUploadToStorage(file, selectedCategory.value)
